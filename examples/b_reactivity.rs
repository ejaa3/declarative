...
#[declarative::view { // first look at the main function below
	gtk::ApplicationWindow window !{ // and then come back here
		...
		// 'bind! initializes and updates, conditionally or not (`if` or `match`)
		// 'bind  does not initialize but updates, conditionally or not (`if` or `match`)
	}
}]
...
fn main() -> glib::ExitCode {
	...
	app.connect_activate(|app| {
		// the following will mutate from a closure at the
		// end of the view that does not implement `FnMut`:
		let main_count = Cell::new(1_u8); // we put an odd number
		
		// we get the count here for a reason explained above (line 34):
		let count = main_count.get();
